...
def test_write_single_asebytes_lmdb(benchmark, dataset, tmp_path):
    name, frames = dataset
    p = str(tmp_path / f"ws_{name}.lmdb")
    db = ASEIO(p)
...
def test_write_single_asebytes_zarr(benchmark, dataset, tmp_path):
    name, frames = dataset
    p = str(tmp_path / f"ws_{name}.zarr")
    db = ASEIO(p)
...
def test_write_single_aselmdb(benchmark, dataset, tmp_path):
    name, frames = dataset
    p = str(tmp_path / f"ws_{name}_aselmdb.lmdb")
...
def test_write_single_znh5md(benchmark, dataset, tmp_path):
    ...
    name, frames = dataset
    p = str(tmp_path / f"ws_{name}_znh5md.h5")
    io = znh5md.IO(filename=p)
...
def test_write_single_extxyz(benchmark, dataset, tmp_path):
    name, frames = dataset
    p = str(tmp_path / f"ws_{name}.extxyz")
...
def test_write_single_sqlite(benchmark, dataset, tmp_path):
    name, frames = dataset
    p = str(tmp_path / f"ws_{name}_sqlite.db")
