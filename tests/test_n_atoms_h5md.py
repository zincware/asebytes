"""Tests for _n_atoms length column in H5MD backend."""
import ase
import numpy as np
import pytest
...
@pytest.fixture
def h5md_path(tmp_path):
    return str(tmp_path / "test.h5")


class TestDtypePreservation:
    def test_numbers_roundtrip_int(self, h5md_path):
    ...
    def test_custom_int_array_preserved(self, h5md_path):
...
class TestVariableLengthRoundtrip:
    def test_varying_atom_counts(self, h5md_path):
        atoms_list = [
    ...
    def test_column_access_varying_lengths(self, h5md_path):
        atoms_list = [
            ase.Atoms("H", positions=[[0, 0, 0]]),
...
class TestFillValues:
    def test_float_arrays_use_nan_fill(self, h5md_path):
    ...
    def test_int_arrays_use_zero_fill(self, h5md_path):
        atoms = ase.Atoms("H2", positions=[[0, 0, 0], [1, 0, 0]])
...
class TestGetMany:
    def test_get_many_varying_atoms(self, h5md_path):
        atoms_list = [
            ase.Atoms("H", positions=[[0, 0, 0]]),
            ase.Atoms("H2O", positions=[[0, 0, 0], [1, 0, 0], [0, 1, 0]]),
            ase.Atoms("H2", positions=[[0, 0, 0], [1, 0, 0]]),
...
class TestMultipleBatchExtend:
    def test_two_batch_extend(self, h5md_path):
        db = ASEIO(h5md_path)
