...
from ._async_protocols import AsyncReadableBackend, AsyncWritableBackend
...
class AsyncASEIO:
    ...
    def __init__(self, backend: AsyncReadableBackend):
    ...
    async def _read_row(
        ...
        return await self._backend.read_row(index, keys)

    async def _read_rows(
        self, indices: list[int], keys: list[str] | None = None
    ) -> list[dict[str, Any] | None]:
        return await self._backend.read_rows(indices, keys)

    async def _read_column(self, key: str, indices: list[int]) -> list[Any]:
        return await self._backend.read_column(key, indices)

    async def _write_row(self, index: int, data: Any) -> None:
        if not isinstance(self._backend, AsyncWritableBackend):
    ...
    async def _get_available_keys(self, index: int) -> list[str]:
        cols = await self._backend.columns(index)
        return cols
