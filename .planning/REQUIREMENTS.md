...
- [ ] **TEST-01**: Contract test suite with parametrized fixtures testing every backend through BlobIO, ObjectIO, and ASEIO facades
- [ ] **TEST-02**: Edge case tests included in contract suite: empty datasets, single-frame, variable particle counts, large arrays, special float values (NaN, inf), empty strings, nested info dicts
